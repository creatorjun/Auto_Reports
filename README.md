...
첫 번째 대시보드는 Jira 프로젝트의 모든 요청 유형을 기본으로 집계합니다. `ISSUE_TYPES`에 등록된 유형은 화면 최상단 토글로 모든 위젯과 상세 모달에서 실시간 포함·제외할 수 있고, 토글에 등록되지 않은 유형은 항상 집계에 포함됩니다. 기존 보고서는 이 집계 계약으로 한 번 생성하거나 갱신해야 토글이 활성화됩니다.
