...
첫 번째 대시보드는 보고서 생성 시 Jira 프로젝트에 현재 등록된 모든 요청 유형을 조회해 화면 최상단 토글에 표시하고, 모든 위젯과 상세 모달에서 실시간 포함·제외할 수 있습니다. `ISSUE_TYPES`는 Jira 유형 목록 조회 실패 시에만 사용하는 안전한 기본값입니다. 기존 보고서는 이 집계 계약으로 한 번 생성하거나 갱신해야 전체 유형 토글이 활성화됩니다.
