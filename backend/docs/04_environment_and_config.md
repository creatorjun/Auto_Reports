...
| `ISSUE_TYPES` | `["인시던트","개선","CVE","서비스 요청","라이선스"]` | 대시보드 토글로 포함·제외할 실제 Jira 이슈 유형 JSON list |
...
첫 번째 대시보드의 w1부터 w14까지 모든 위젯은 Jira 프로젝트의 모든 이슈 유형을 포함해 생성됩니다. `ISSUE_TYPES`에 등록된 유형은 연간·월별·SLA 유형별 분해값으로 저장되어 화면 최상단 토글에서 즉시 포함·제외할 수 있습니다. `ISSUE_TYPES`에 없는 유형은 `always_included` 집계 또는 원본 상세에 유지되어 토글 상태와 관계없이 항상 모든 위젯에 포함됩니다. 이 데이터 계약이 추가되기 전에 생성된 보고서는 새로 생성하거나 갱신해야 토글을 사용할 수 있습니다.
