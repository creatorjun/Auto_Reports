...
export default function AiSummaryCard({ ai }: { ai: AiAnalysis }) {
  ...
  return (
    <div className={`rounded-2xl border ${cfg.border} ${cfg.bg} p-6 shadow-apple-sm`}>
      <div className="flex items-center gap-2.5 mb-4">
        <span className={`w-2 h-2 rounded-full ${cfg.dot}`} />
        <h3 className="text-[15px] font-semibold text-apple-dark">AI 종합 분석</h3>
        <span className={cfg.badge}>{cfg.label}</span>
      </div>
      <p className="text-[13px] text-apple-dark/80 leading-relaxed mb-5">{ai.summary}</p>
      <div className="grid grid-cols-2 gap-6">
        <div>
          <p className="text-[11px] font-semibold text-red-600 uppercase tracking-wider mb-2.5">주목 리스크</p>
        ...
        <div>
          <p className="text-[11px] font-semibold text-brand-600 uppercase tracking-wider mb-2.5">다음 주 권고사항</p>
          <ul className="space-y-1.5">
