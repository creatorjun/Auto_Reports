...
export default function DataRequestModal({ issues, total, onClose }: Props) {
  ...
  return (
    <IssueModalShell title="자료 요청 중" subtitle={`SLA 초과 후 자료 대기 · 전체 ${total}건 (오래된 순)`} onClose={onClose}>
          ...
          <tbody className="divide-y divide-apple-divider/40">
            {issues.map((d) => (
              <tr key={d.key} onClick={() => window.open(`${jiraBrowse}/${d.key}`, '_blank', 'noreferrer')} className="hover:bg-apple-gray/50 transition-colors duration-150 cursor-pointer">
                ...
                <td className="py-2.5 whitespace-nowrap"><span className={MODAL_CLS.elapsedCell}>{d.elapsed_days}일</span></td>
              </tr>
            ))}
          </tbody>
        </table>
