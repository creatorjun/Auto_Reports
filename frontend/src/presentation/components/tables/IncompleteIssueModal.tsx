    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl mx-4 max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-apple-divider/60">
          <div>
            <h2 className="text-[15px] font-semibold text-apple-dark">미완료 이슈 목록</h2>
            <p className="text-[12px] text-apple-light mt-0.5">현재 진행 중인 미완료 이슈 · 전체 {total}건 (경과일 순)</p>
          </div>
          <button
            onClick={onClose}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-apple-divider/60">
                  {['이슈 번호', '제목', '유형', '현재 상태', '생성일시', '경과일'].map((h) => (
                    <th
                      key={h}
                      className="text-left pb-3 text-[11px] font-semibold text-apple-light uppercase tracking-wider whitespace-nowrap pr-4"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-apple-divider/40">
                {issues.map((d) => (
                  ...
                  >
                    <td className="py-2.5 pr-4 text-[12px] font-mono font-medium text-brand-600 whitespace-nowrap">
                      {d.key}
                    </td>
                    <td className="py-2.5 text-[12px] text-apple-dark/80 max-w-xs truncate pr-4">
                      {d.summary}
                    </td>
                    <td className="py-2.5 text-[12px] text-apple-light whitespace-nowrap pr-4">
                      {d.type}
                    </td>
                    <td className="py-2.5 whitespace-nowrap pr-4">{statusBadge(d.status)}</td>
                    <td className="py-2.5 text-[12px] text-apple-light tabular-nums whitespace-nowrap pr-4">
                      {d.created}
                    </td>
