      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-5 flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex gap-3">
              <div className="flex-1">
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 rounded-lg border text-[13px] text-apple-dark bg-white
                    transition-colors outline-none
                    ${
                      mode === 'custom' && startDate
                        ? 'border-brand-400 ring-1 ring-brand-200'
                        : 'border-apple-divider'
                    }
            ...
            {dateError && (
              <p className="text-[12px] text-red-500">⚠️ 완료일은 시작일보다 많아야 합니다.</p>
            )}
            ...
            {(startDate || endDate) && !dateError && (
              <div className="flex items-center gap-2 px-3 py-2 bg-brand-50 rounded-lg border border-brand-100">
                <span className="text-[11px] text-brand-700">
                  확정 기간: <strong>{startDate || '—'}</strong> – <strong>{endDate || '—'}</strong>
                </span>
              </div>
            )}
          </div>
        ...
        <div className="px-6 py-4 border-t border-apple-divider/60 flex items-center justify-between gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl text-[13px] font-medium bg-apple-gray hover:bg-apple-divider text-apple-dark transition-colors"
          >
            취소
          </button>
          <button
