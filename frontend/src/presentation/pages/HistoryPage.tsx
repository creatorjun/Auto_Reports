...
export default function HistoryPage() {
  ...
  return (
    <div className="space-y-4 md:space-y-5 max-w-[1400px]">
      <div>
        <h1 className="text-[18px] md:text-[22px] font-semibold text-apple-dark tracking-tight">보고서 히스토리</h1>
        <p className="text-[12px] md:text-[13px] text-apple-light mt-1">자동 생성된 TAC 주간 보고서 목록</p>
      </div>
              ...
              {(data ?? []).map((r) => (
                <tr key={r.id} className="hover:bg-apple-gray/60 transition-colors duration-150">
                  ...
                  <td className="px-6 py-3.5 text-right">
                    <Link to={`/reports/${r.id}`} className="text-[12px] text-brand-600 hover:text-brand-700 font-medium transition-colors">
                      상세 보기
                    </Link>
                  </td>
                </tr>
