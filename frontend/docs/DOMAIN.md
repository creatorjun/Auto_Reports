...
`ReportDetail`은 summary 필드에 widget map과 선택적 AI 분석을 추가합니다. 서버 widget의 서로 다른 breakdown 구조를 React component 내부에서 즉석 추론하지 않고 `useDashboardData`가 `Dashboard.ts`의 표시 모델로 변환합니다. 대시보드와 연간 보고서는 Jira 요청 유형 `라이선스`를 항상 제외하며, 요청 유형 토글에도 표시하지 않습니다. 보고서 생성 시 라이선스를 제외한 비하위 작업 유형을 `issue_types`에 저장하며, 화면 최상단 요청 유형 토글은 이 목록의 상세·유형별 집계를 조합해 카드, 모달, 월별 차트, SLA 차트, 분석 차트와 최근 이슈 현황을 즉시 다시 계산합니다. 같은 영역의 현재 상태 토글은 보고서 생성 시 저장한 `by_status_type` 교차 집계를 사용해 요청 유형 및 반기 선택과 동시에 적용됩니다. 상태 교차 집계가 없는 기존 보고서에서는 상태 필터를 비활성화하며, 보고서를 갱신하면 사용할 수 있습니다. 파트너 관리 화면도 최신 보고서의 동일한 필터 계약을 사용해 조직·멤버 이슈 목록에 적용합니다. 기존 저장 보고서도 유형별 분해값으로 라이선스를 차감합니다. 상반기·하반기 버튼은 보고서 연도를 기준으로 월별 데이터와 상세 이슈의 생성일·해결일을 교차 필터링하며, 선택하지 않으면 12개월 전체를 표시합니다. 프로젝트 유형 목록 밖의 데이터는 `always_included` 집계 또는 상세의 미등록 유형 판별을 통해 계속 포함하지만, 라이선스는 예외로 항상 제외됩니다. 요청 유형·현재 상태·반기 중 하나를 선택한 동안에는 전체 데이터를 기준으로 생성된 AI 분석을 숨깁니다. w3 생성·완료 건수에는 보고서 쿼리의 시작일과 종료일을 포함한 기간 일수가 표시됩니다. w7 미완료 이슈 상세는 `WorkTypeOpenWidget`으로 분류되어 지원 요청, 개선 요청, 인시던트 보고, CVE의 현재 열린 요청 건수를 제공합니다.
