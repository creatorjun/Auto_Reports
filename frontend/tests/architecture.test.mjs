...
test('annual reports render every redeployment dashboard view', () => {
  ...
  assert.match(section, /CATEGORICAL_CHART_COLORS/)
...
test('partner management filters organizations members and issues independently', () => {
  ...
  assert.match(orgPanel, /sortPartnerOrganizationsByIssueCount\(/)
  assert.match(orgPanel, /org\.issue_count\.toLocaleString\('ko-KR'\)/)
...
test('dashboard request type current status and semester filters drive every widget view', () => {
  ...
  assert.match(filter, /useState\(false\)/)
  assert.match(filter, /aria-expanded=\{expanded\}/)
  assert.match(filter, /aria-controls="issue-filter-options"/)
  assert.match(filter, /필터 펼치기/)
  assert.match(filter, /필터 접기/)
...
test('SLA dashboard exposes issue activity and expandable recent comments', () => {
  ...
  assert.match(table, /최신 5개부터 표시/)
  assert.match(table, /최신 \$\{data\.length\}개 · 최신순/)
  assert.match(table, /table-fixed/)
  assert.match(table, /object-left-top/)
